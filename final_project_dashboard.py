...
avg_age =df["age"].mean()
avg_bmi = df["bmi"].mean()
avg_alcoholconsumption = df["alcoholconsumption"].mean()
avg_physicalactivity = df["physicalactivity"].mean()
avg_mmsescore = df["mmse"].mean()

col1, col2, col3 = st.columns(3)
col1.metric(label="\# of Patients", value=f"{num_patients:,}")
col2.metric(label="Avg. Age", value=f"{avg_age:,.0f}")
col3.metric(label="Avg. MMSE Score", value=f"{avg_mmsescore:,.0f}")
...
fig_histo_age = px.histogram(data_frame=df, x='age', color='diagnosis',
...
symptom_data = {col: (df[col] == 'yes').mean() * 100 for col in symptom_columns}
...
fig_box_smoking = px.box(df, x='smoking', y='mmse', color='diet_quality', 
...
total = len(df)
percent_data = (df.groupby(['diet_quality', 'activity_level', 
...
mean_mmse = df.groupby(['educationlevel', 'diagnosis'], as_index=False)['mmse'].mean()
...
#sunburst data time
total = len(df)
percent_data = (df.groupby(['bmi_level', 'cholesterol_total_level', 'diagnosis'])
                .size()
                .reset_index(name='count'))
percent_data['percentage'] = (percent_data['count'] / total * 100).round(1)
...
bp_melted_diag = df.melt(id_vars=['diagnosis'], 
...
ethnicity_diagnosis = df.groupby(['ethnicity', 'diagnosis']).size().reset_index(name='count')
...
depression_by_age = df.groupby('age_group')['depression'].value_counts(normalize=True).rename('percentage').reset_index()
...
headinjury_by_age = df.groupby('age_group')['headinjury'].value_counts(normalize=True).rename('percentage').reset_index()
